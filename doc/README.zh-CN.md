...
会以 `roslaunch --wait` 启动，先阻塞直到 master 可连接，再开始启动。此 **boot gate**
没有 timeout、也没有旋钮 -- 等待永远是无限的。启动顺序不再重要 -- slave 可以早于
master 启动（例如开机时自动启动），仍会在 master 出现后干净地注册，而不会变成一个
未注册的僵尸节点。
...
`config/realsense/yaml/none.yaml`，一个**空文件**，因此 runtime image 会串流原厂
...
```bash
ln -sf config/realsense/yaml/usb2_640x480p15fps.yaml camera.yaml  # 启用 USB 2 profile
ln -sf config/realsense/yaml/none.yaml camera.yaml   # 回到原厂默认
...
要 remap 输出(例如下游需要自己的 topic 名称):复制
`config/realsense/rs_camera_remap.example.launch`,改那两行
...
| `sensor_options.example.yaml` | `disparity,temporal` | 复制用模板。加上 `rgb_camera` / `stereo_module` 区块（曝光、增益、激光功率），值为驱动默认 -- 同一个 profile 文件同时触及 filter 与两个传感器 |
...
构建期自动测试详见 [TEST.md](test/TEST.md)；实机相机测试见 [CAMERA.zh-CN.md](CAMERA.zh-CN.md)；动态校正工具见 [CALIBRATION.zh-CN.md](CALIBRATION.zh-CN.md)。
...
│       │   └── sensor_options.example.yaml  # 复制用模板：filter + RGB/stereo 传感器选项（#149）
...
│   ├── README.ja.md             # 日文
│   ├── adr/                     # 架构决策记录（ADR）
│   ├── CAMERA.md                # 实机相机手动测试（另有 .zh-TW/.zh-CN/.ja）
│   ├── CALIBRATION.md           # 动态校正工具说明（另有 .zh-TW/.zh-CN/.ja）
...
└── test/
    └── smoke/                   # 仓库自有的 bats 测试
        ...
        ├── profile_assert.bats       # profile 参数确实送达节点（#149）
