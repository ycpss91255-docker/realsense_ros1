...
`config/realsense/yaml/custom/none.yaml`，一个**空文件**，因此 runtime image 会串流原厂
...
```bash
ln -sf config/realsense/yaml/custom/usb2_640x480p15fps.yaml camera.yaml  # 启用 USB 2 profile
ln -sf config/realsense/yaml/custom/none.yaml camera.yaml   # 回到原厂默认
...
`config/realsense/launch/example/rs_camera_remap.example.launch`,改那两行
...
profile 放在 `config/realsense/yaml/custom/`。命名为 `<link>_<WxH>p<fps>fps.yaml`：
每个 color 分辨率一个文件，采用该分辨率在该 link 上的最大 fps。depth 一律 1280x720
（D455 的最高 depth 分辨率），上限 30 fps（720p depth 的上限）。每个 profile 都关闭
IR（`enable_infra1/2`）与 IMU（`enable_gyro/accel`）；对齐深度开启。
...
`usb3_*` profile 已在 D455 通过 USB 3 link 用 `rs-enumerate-devices` 验证。`usb2_*`
profile **尚未验证**：USB 2 白名单从未列举过（相机当时接在 USB 3 link 上），因此 720p
...
#### `yaml/official/` + `udev/` -- 源自上游的文件

源自上游的文件与我们自己的 `yaml/custom/` profile 分开存放：

| 文件 | 来源 | 漂移检查 |
|------|------|----------|
| `udev/99-realsense-libusb.rules` | 自 `librealsense` SDK 的 `config/99-realsense-libusb.rules`（固定 `LIBREALSENSE_VERSION`）**verbatim** vendored | `script/check_udev_rules_sync.sh` |
| `yaml/official/config.yaml` | ROS 2 上游示例配置的**同义 ROS 1 移植版**（非 verbatim -- ROS 1 realsense-ros 上游不附 config YAML） | （无） |

udev 规则就是 host USB 访问规则（见上一节）；Dockerfile 也会把它烤进 image 的
`/etc/udev/rules.d/`。

`config.yaml` 翻成 ROS 1 参数名称（`rgb_camera.color_profile: 1280x720x15` ->
...
```text
realsense_ros1/
├── Dockerfile                   # 多阶段构建（FROM 预构建的 librealsense SDK 镜像）
├── docker/
│   └── librealsense/
│       └── Dockerfile           # 预构建 librealsense SDK 来源（构建一次，发布到 GHCR）
├── LICENSE
├── README.md
...
├── camera.yaml -> config/realsense/yaml/custom/none.yaml  # symlink（当前生效的相机配置；默认为 stock）
├── config/
│   ├── docker/
...
│       ├── yaml/               # 相机 profile + 参考 YAML
│       │   ├── official/       # 源自上游
│       │   │   └── config.yaml # ROS 2 示例的同义 ROS 1 移植版（跨 repo 对齐）
│       │   └── custom/         # 我们的相机配置（ROS 1 参数格式）；none + USB2/USB3 预设
│       │       ├── none.yaml               # 空文件 = stock 上游默认（默认）
│       │       ├── usb3_1280x720p30fps.yaml  # 已在 D455 验证（USB 3）
│       │       ├── usb3_848x480p60fps.yaml   # 已在 D455 验证（USB 3）
│       │       ├── usb3_640x480p60fps.yaml   # 已在 D455 验证（USB 3）
│       │       ├── usb3_424x240p90fps.yaml   # 已在 D455 验证（USB 3）
...
│   └── build-librealsense.yaml  # 发布预构建 librealsense SDK 镜像到 GHCR
└── test/
    └── smoke/                   # 仓库自有的 bats 测试
        ├── ros_env.bats
