...
`config/realsense/official/` 放源自上游的文件，与我们自己的 `custom/` profile 分开：

| 文件 | 来源 | 漂移检查 |
|------|------|----------|
| `99-realsense-libusb.rules` | 自 `librealsense` SDK 的 `config/99-realsense-libusb.rules`（固定 `LIBREALSENSE_VERSION`）**verbatim** vendored | `script/check_udev_rules_sync.sh` |
| `config.yaml` | ROS 2 上游示例配置的**同义 ROS 1 移植版**（非 verbatim -- ROS 1 realsense-ros 上游不附 config YAML） | （无） |
...
├── camera.yaml -> config/realsense/custom/none.yaml  # symlink（当前生效的相机配置；默认为 stock）
├── config/
│   ├── docker/
