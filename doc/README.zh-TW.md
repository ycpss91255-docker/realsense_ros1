...
`config/realsense/yaml/custom/none.yaml`，一個**空檔**，因此 runtime image 會串流原廠
...
```bash
ln -sf config/realsense/yaml/custom/usb2_640x480p15fps.yaml camera.yaml   # 啟用 USB 2 profile
ln -sf config/realsense/yaml/custom/none.yaml camera.yaml   # 回到原廠預設
...
`config/realsense/launch/internal/` 自有 `rs_camera_config.launch`(baked 成
...
`config/realsense/launch/example/rs_camera_remap.example.launch`,改那兩行
...
profile 放在 `config/realsense/yaml/custom/`。命名為
...
#### `yaml/official/` + `udev/` -- 源自上游的檔案

源自上游的檔案與我們自己的 `yaml/custom/` profile 分開存放:
...
| `yaml/official/config.yaml` | ROS 2 上游範例設定的**同義 ROS 1 移植版**（非 verbatim -- ROS 1 realsense-ros 上游不附 config YAML） | （無） |

udev 規則就是 host USB 存取規則（見上一節）；Dockerfile 也會把它烤進 image 的
`/etc/udev/rules.d/`。

`config.yaml` 翻成 ROS 1 參數名稱（`rgb_camera.color_profile: 1280x720x15` ->
`color_width/height/fps`、`align_depth.enable` -> `align_depth` ...）；ROS 2 專屬的
`publish_tf` / `tf_publish_rate` 已省略（ROS 1 realsense-ros 預設就會發布靜態 TF
樹）。它沒有綁任何 build arg；把 `camera.yaml` 指向它（或
`--build-arg CAMERA_CONFIG=config/realsense/yaml/official/config.yaml`）即可使用。
...
├── camera.yaml -> config/realsense/yaml/custom/none.yaml  # symlink（作用中的相機設定檔；預設為 stock）
├── config/
│   ├── docker/
...
│       ├── yaml/               # 相機 profile + 參考 YAML
│       │   ├── official/       # 源自上游
│       │   │   └── config.yaml # ROS 2 範例的同義 ROS 1 移植版（跨 repo 對齊）
│       │   └── custom/         # 我們的相機設定檔（ROS 1 參數格式）；none + USB2/USB3 preset
