...
#### `official/` -- ROS 2 範例的 ROS 1 移植版

ROS 1 `realsense-ros` **上游不附 config YAML**（只透過 `rs_*.launch` 參數調整），因此
不像 ROS 2 sibling 有官方檔可 vendored、也沒有 drift-check。為與 ROS 2 對齊，
`config/realsense/official/config.yaml` 保留一份 ROS 2 上游範例設定的**同義 ROS 1
移植版**，翻成 ROS 1 參數名稱（`rgb_camera.color_profile: 1280x720x15` ->
...
│       ├── official/           # ROS 2 上游範例的 ROS 1 移植版（無上游 drift-check）
│       │   └── config.yaml     # 同義 ROS 1 移植版，為跨 repo 對齊而保留
