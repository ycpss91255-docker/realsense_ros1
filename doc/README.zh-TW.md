...
`config/realsense/custom/none.yaml`，一個**空檔**，因此 runtime image 會串流原廠
...
```bash
ln -sf config/realsense/custom/usb2.yaml camera.yaml   # 啟用 USB 2 profile
ln -sf config/realsense/custom/none.yaml camera.yaml   # 回到原廠預設
...
| `usb2.yaml` | USB 2.x 後備：color 640x480@15、depth 480x270@15、對齊深度開啟、IR + IMU 關閉。 |

D435/D455 接 USB 2 連線時無法承載原廠的 640x480x30 color + depth（相機會協商連線
但在 30 fps 下送出 **0 frame**），因此 `usb2.yaml` 把頻寬縮到可塞進 480 Mbps 連線：
color 與 depth 都降到 15 fps、depth 降到 480x270，並關閉連線無法負擔的 IR
（`enable_infra1/2`）與 IMU（`enable_gyro/accel`）串流；對齊深度維持開啟。已在 USB 3
埠退回 USB 2 的 Raspberry Pi 5（arm64）上驗證。

#### `official/` -- 源自上游的檔案

`config/realsense/official/` 放源自上游的檔案，與我們自己的 `custom/` profile 分開：

| 檔案 | 來源 | 漂移檢查 |
|------|------|----------|
| `99-realsense-libusb.rules` | 自 `librealsense` SDK 的 `config/99-realsense-libusb.rules`（釘選 `LIBREALSENSE_VERSION`）**verbatim** vendored | `script/check_udev_rules_sync.sh` |
| `config.yaml` | ROS 2 上游範例設定的**同義 ROS 1 移植版**（非 verbatim -- ROS 1 realsense-ros 上游不附 config YAML） | （無） |
