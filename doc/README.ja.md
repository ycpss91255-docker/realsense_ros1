...
`config/realsense/official/` は上流由来のファイルを、独自の `custom/` profile とは分けて
保持します：

| ファイル | 由来 | ドリフトチェック |
|----------|------|-------------------|
| `99-realsense-libusb.rules` | `librealsense` SDK の `config/99-realsense-libusb.rules`（ピン留め `LIBREALSENSE_VERSION`）から **verbatim** vendored | `script/check_udev_rules_sync.sh` |
| `config.yaml` | ROS 2 上流サンプル設定の**同義 ROS 1 移植版**（verbatim ではない -- ROS 1 realsense-ros は上流に config YAML なし） | （なし） |
